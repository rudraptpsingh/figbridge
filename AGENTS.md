...
Context for future Codex sessions working in this repo. Keep short and current.
...
Codex Desktop respawns npx on every launch → users get new versions for free.
...
- **Clean shutdown** (since 0.1.6): `server.js` exits on stdin end/close and SIGTERM/SIGINT/SIGHUP, closing the HTTP server first. Prevents zombie processes piling up when Codex Desktop respawns us.
- **`figbridge-mcp doctor`** (since 0.1.6): reaps orphan figbridge-mcp processes (SIGTERM then SIGKILL) and probes 7331..7340 for live bridges. `init` / `update` run the reaper before rewriting the config so the next Codex launch lands on a clean :7331.
